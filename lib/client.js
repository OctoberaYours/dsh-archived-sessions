window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 【本地诊断 2026-09-24】原值：["slots","locale","sessions","workspaces"]
		//
		// 现象（DSH Desktop 0.1.7-rc.1）：设置页「会话管理」占位在、点进去整片空白；
		// 会话头部也没有删除按钮；**控制台一条报错都没有**。
		//
		// 判断：`inject` 是**严格门禁** —— 声明了就必须有同名 cordis Service，否则
		// apply 被永久挂起（静默）。而 0.1.7 的客户端服务表里（全量扫 `super(ctx,"…")`）
		// 只有 `slots` 在列；`sessions` / `workspaces` 走的是 `ctx.get("sessions")` 与
		// `hooks: { sessions: ctx.sessions.list }`，**不是 Service 子类**。
		//
		// 旁证：同机正常的 po06 客户端只声明 `['slots','locale']`；而它自己的注释里
		// 记着同类真机事故：「读 `ctx.locale` 而没在 `exports.inject` 里声明 ⇒ 整页
		// "Failed to load plugins"」—— 说明这个门禁是严格的、且失败很难归因。
		//
		// 下面先只留确实存在的服务，验证「apply 是否被 sessions/workspaces 挂住」。
 ...
 * DSH 0.1.5-rc.1 exposes no workspaces.refresh() at all and no root-level
 * sessions.refresh(), so every call is feature-detected: both stores are
 * push-updated, and a missing method must never throw inside a click handler.
 */
async function refreshSessionLists(ctx) {
  try {
    const sessions = ctx.sessions;
    if (sessions && typeof sessions.refresh === "function") await sessions.refresh();
  } catch { /* best effort: the store is push-updated */ }
  try {
    const workspaces = ctx.workspaces;
    if (workspaces && typeof workspaces.refresh === "function") await workspaces.refresh();
  } catch { /* best effort: the store is push-updated */ }
}
function apply(ctx) {
