window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * 【本地兼容 2026-09-24】DSH 0.1.7 改了 primitives 的图标命名。
		 *
		 * 0.1.6 及以前：图标名带**尺寸后缀** —— `IconTrashOutline16`、`IconTriangleRightFill14`。
		 * 0.1.7-rc.1：尺寸后缀被去掉，改为**字重变体** —— `IconTrashOutline` + `…Regular` /
		 *   `…Medium` / `…Artwork`。实测（扫 asar 内 primitives/lib/index.js）：
		 *     IconCheckOutline16        0 次   IconCheckOutline        14 次
		 *     IconFolderOpenOutline16   0 次   IconFolderOpen         18 次
		 *     IconTrashOutline16        0 次   IconTrashOutline        9 次
		 *     IconTriangleRightFill14   0 次   IconTriangleRightFill   9 次
		 *
		 * 后果不是"图标不显示"那么轻：拿到 `undefined` 之后它被当**组件**渲染 ⇒
		 * `Minified React error #130`（Element type is invalid）⇒ **整个 slot 条目崩溃**。
		 * 真机表现：会话头部删除按钮处报
		 *   `slot entry crashed in 'conversation.session.header.actions'`；
		 * 设置页「会话管理」点了没反应、面板整片空白且**无任何报错**。
		 *
		 * 所以这里做一层**降级映射**：优先用带尺寸后缀的旧名（0.1.6），拿不到再退回
		 * 不带后缀的新名（0.1.7），两者都没有则给一个合法的空占位组件 ——
		 * **绝不让 `undefined` 流到渲染层**（那才是 #130 的成因）。
		 */
		const P = _deepseek_ai_dsh_client_ui_primitives;
		const NullIcon = () => null;
		const pickIcon = (legacy, modern) => {
			const cand = [legacy, modern, modern + "Regular", modern + "Medium"];
			for (const n of cand) if (P && typeof P[n] === "function") return P[n];
			return NullIcon;
		};
		const IconCheckOutline16 = pickIcon("IconCheckOutline16", "IconCheckOutline");
		const IconFolderOpenOutline16 = pickIcon("IconFolderOpenOutline16", "IconFolderOpen");
		const IconTrashOutline16 = pickIcon("IconTrashOutline16", "IconTrashOutline");
		const IconTriangleRightFill14 = pickIcon("IconTriangleRightFill14", "IconTriangleRightFill");
		//#region css
		...
		/* 【本地修复 2026-09-24】inject 声明 —— 这里之前被误删过两个服务，代价很大。
		 *
		 * 事实链（全部实测，别再凭"扫不到 super(ctx,…)"推断）：
		 *
		 * 1) cordis 4 的 `inject` 是**访问门禁**，不只是启动依赖声明。未声明的服务用
		 *    `ctx.<name>` 读取会**直接抛错**：
		 *        Error: cannot get property "sessions" without inject
		 *    实测代码见本仓库历史；`ctx.get("<name>")` 才是无门禁的宽容读取。
		 *
		 * 2) `sessions` 与 `workspaces` 在 0.1.7 客户端**确实是 cordis Service**：
		 ...
		 *    官方 `dsh-client-ui-workspace` 的 inject 就是
		 *        ["slots","sessions","workspaces","locale","remote",…]
		 *    先前"只扫到 slots"是因为扫描脚本只匹配了 `super(ctx, "name")` 这一种写法。
		 *
		 * 3) 后果：`refreshSessionLists()` 里 `ctx.sessions` 抛错，而那里是
		 *    `try{}catch{}` 静默兜底 ⇒ 删除后客户端**从不刷新**会话列表。
		 *    真机表现：删掉的会话仍留在侧边栏，因已从工作区摘除而掉进「未分组」，
		 *    点开报 `session "…" not found`。
		 *
		 * 结论：这两个服务必须声明。
		 */
				...
				children: [(0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						hasKids && (0, react_jsx_runtime.jsx)("button", {
							...
							children: (0, react_jsx_runtime.jsx)(IconTriangleRightFill14, {})
						}),
						(0, react_jsx_runtime.jsx)("span", {
							...
							children: row.current ? (0, react_jsx_runtime.jsx)("span", { className: pcss.currentBadge, children: currentText }) : (0, react_jsx_runtime.jsx)("span", {
								...
								children: isSelected && (0, react_jsx_runtime.jsx)(IconCheckOutline16, { size: 12 })
							})
						}),
													...
													children: [
														(0, react_jsx_runtime.jsxs)("label", {
															...
															children: [(0, react_jsx_runtime.jsx)("input", { type: "checkbox", checked: flist.every((f) => selectedFiles.has(f.path)), onChange: () => toggleFolderFiles(flist), title: t("selectAll") }), (0, react_jsx_runtime.jsx)("button", {
																...
																children: (0, react_jsx_runtime.jsx)("span", { style: { display: "inline-flex", transform: expandedFileDirs.has(folder) ? "rotate(90deg)" : "rotate(0deg)", transition: "transform 0.15s ease" }, children: (0, react_jsx_runtime.jsx)(IconTriangleRightFill14, {}) })
					...
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							(0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Button, {
								...
								children: (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, {
									children: [(0, react_jsx_runtime.jsx)(IconFolderOpenOutline16, { size: 14 }), " ", t("openFolder")]
								})
							})
						]
					}),
												...
												children: [
													(0, react_jsx_runtime.jsxs)("label", {
														...
														children: [(0, react_jsx_runtime.jsx)("input", { type: "checkbox", checked: deleteFilePaths.has(dir), disabled: deleting, onChange: (e) => setDeleteFilePaths((prev) => { const next = new Set(prev); if (e.target.checked) next.add(dir); else next.delete(dir); return next; }) }), (0, react_jsx_runtime.jsx)("button", {
															...
															children: (0, react_jsx_runtime.jsx)("span", { style: { display: "inline-flex", transform: expandedFileDirs.has(dir) ? "rotate(90deg)" : "rotate(0deg)", transition: "transform 0.15s ease" }, children: (0, react_jsx_runtime.jsx)(IconTriangleRightFill14, {}) })
			...
			return (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, {
				children: [
					(0, react_jsx_runtime.jsx)("button", {
						...
						children: (0, react_jsx_runtime.jsx)(IconTrashOutline16, {})
					}),
					(0, react_jsx_runtime.jsxs)(_deepseek_ai_dsh_client_ui_primitives.Modal, {
						open: open,
		...
		/**
 * Refresh the session / workspace lists after an archive or a delete.
 *
 * 【本地修复 2026-09-24】旧实现有两个叠加缺陷，导致"删了但列表不更新"：
 *
 * 1) 用 `ctx.sessions` / `ctx.workspaces` 读取服务，但这两个名字没写进 `inject`。
 *    cordis 4 对未声明的服务属性访问**直接抛错**：
 *        Error: cannot get property "sessions" without inject
 *    下面每个调用都裹在 `try{}catch{}` 里做"best effort"，于是异常被静默吞掉 ——
 *    看起来毫无问题，实际一次都没刷新过。
 *
 * 2) 即便拿到服务，`refresh()` 的语义也不对。0.1.7 的会话列表是**客户端 store**
 *    （`ctx.sessions.list`，由 api-session-controller 的 manager 维护），它由宿主
 *    主动推送 `api-session/removed` / `api-session/added` 帧来增删行。插件是在
 *    **绕开官方 API 直接删磁盘**，所以没有任何帧发出，store 里那一行会一直留着。
 *    这正是"删掉的会话掉进未分组、点开报 not found"的成因：宿主的 workspace
 *    detach 生效了（所以脱离原分组），但列表行没有被移除（于是进 stray 桶）。
 *
 * 正确做法：删除后**显式重新拉取基线**（`refresh()` → `manager.refreshList()`，
 * 它以宿主当前的持久化列表为准重建 store），再顺带刷新工作区分组。
 * 两个服务都已在上方 `inject` 里声明。
 *
 * 全部调用仍然 feature-detect + 兜底：老版本 Harness 上缺方法不应在点击处理器里抛错。
 */
async function refreshSessionLists(ctx) {
	const attempts = [];
	// 会话列表是权威：refresh() → manager.refreshList() 以宿主当前持久化列表重建 store，
	// 从而移除已删除的行（宿主不会为"非 live 的已删会话"推 api-session/removed 帧）。
	try {
		const sessions = ctx.sessions;
		if (sessions !== void 0 && typeof sessions.refresh === "function") {
			attempts.push(Promise.resolve(sessions.refresh()));
		}
	} catch { /* 服务缺失：store 仍会由宿主帧推送更新 */ }
	// 工作区分组：删除时宿主做了 detachSession。0.1.7 的 ctx.workspaces 上没有 refresh()，
	// 状态在 this.list（Workspace 快照 model）上，用 invalidate(true) 立即重取快照。
	try {
		const workspaces = ctx.workspaces;
		if (workspaces !== void 0) {
			const model = workspaces.list ?? workspaces;
			if (typeof model.refresh === "function") attempts.push(Promise.resolve(model.refresh()));
			else if (typeof model.invalidate === "function") attempts.push(Promise.resolve(model.invalidate(true)));
		}
	} catch { /* best effort */ }
...
function apply(ctx) {
			...
			// LOCAL PATCH: keep this page, but register it under a DIFFERENT slot id.
			// DSH 0.1.6 ships its own archived-session settings page
			// (@deepseek-ai/dsh-client-ui-settings-unarchive-sessions) which registers
			// "settings.section" with id "archived-sessions" -- exactly the id this plugin
			// used, so the two would collide on the same slot. Running both side by side is
			// deliberate: the official page ("已归档会话") only UNARCHIVES one row at a time,
			// while this one ("会话管理") adds bulk select, archive, unarchive and delete.
			ctx.slots.inject("settings.section", () => ctx.slots.register({
				name: "settings.section",
				id: "archived-sessions-manager",
				order: 200,
				label: () => t("nav"),
				locale: NS,
				inject: () => ({
					refresh: async () => {
						await refreshSessionLists(ctx);
					}
				})
