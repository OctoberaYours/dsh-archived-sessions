...
async function deleteSessionSingle(ctx, sessionId, options = {}) {
	...
	const removeLog = async () => {
		...
		// 目录内可能同时存在多个 format generation 的日志；整体删目录交由调用方，这里删掉当前代。
		const entries = await readdir(sessionDirPath).catch(() => []);
		for (const entry of entries) {
			if (!/\.jsonl(\.zstd)?$/i.test(entry)) continue;
 ...
 * rc.2 移除了该内部结构，改为先释放 handle 再短暂让出事件循环，等挂起的写收尾。
...
async function cleanUpRetiredDirs(ctx, entries) {
	...
	for (const { sessionId, dir } of entries) {
		...
		if (retirement !== void 0) {
			...
			// rc.2: 无 retirements 表。detach 广播 session/disposed 后，persistence
			// 的尾部 flush 是异步的；让出若干轮事件循环 + I/O，给写入器收尾的机会，
			// 否则紧随其后的 rm 可能被重建的目录"复活"。
