...
// decodeStorageRecord is only needed by the raw-artifact fallback, and it has
// moved out of @deepseek-ai/dsh-session on newer cores. A static named import of
// a missing export kills the whole module at load time, so resolve it lazily and
// tolerate its absence instead.
let decodeStorageRecord;
await import("@deepseek-ai/dsh-session").then((namespace) => {
	decodeStorageRecord = typeof namespace?.decodeStorageRecord === "function" ? namespace.decodeStorageRecord : void 0;
}, () => {});
...
// -- persistence read compatibility (rc.8 <-> 0.1.3-alpha.2) -----------------
// rc.8: sessionPersistence.list(signal) yields flat SessionHeader[], and a whole
// log is read through inspect()/load(). 0.1.3-alpha.2: list(options) yields
// SessionPersistenceSnapshot[] with the header nested under .header (the core
// itself does `list().map((s) => s.header)`), those read primitives are gone, and
// a stored log is read through open(id, 'read') + handle.read(). Normalize both
// spellings here so one build of this plugin is correct on either core.
/** Unwrap one listing item into a flat session header, whichever shape it has. */
function toSessionHeader(item) {
	if (item === null || typeof item !== "object") return void 0;
	const header = typeof item.id === "string" ? item : item.header;
	if (header === null || typeof header !== "object") return void 0;
	return typeof header.id === "string" ? header : void 0;
}
/** Every persisted session header, in whatever shape this core's listing returns. */
async function listStoredHeaders(persistence) {
	if (persistence === void 0) return [];
	const list = typeof persistence.listSnapshots === "function" ? persistence.listSnapshots : typeof persistence.list === "function" ? persistence.list : void 0;
	if (list === void 0) return [];
	const raw = await list.call(persistence);
	const headers = [];
	for (const item of raw ?? []) {
		const header = toSessionHeader(item);
		if (header !== void 0) headers.push(header);
	}
	return headers;
}
/** Whether a persistence failure means "this session is simply not stored". */
function isNotFoundLike(error) {
	return /NotFoundError|not found|does not exist/i.test(`${error?.name ?? ""} ${error?.message ?? ""}`);
}
/** The plugin's own 404 (a raw core error carries no status and would surface as 500). */
...
/** Read one stored session's header + events across core generations. */
async function readStoredSession(persistence, sessionId, signal) {
	// Spelled via a local binding on purpose: this helper is inserted before the
	// call sites that patch next, and must not collide with their anchors.
	const inspect = persistence.inspect;
	if (typeof inspect === "function") return await inspect.call(persistence, sessionId, signal);
	const load = persistence.load;
	if (typeof load === "function") return await load.call(persistence, sessionId);
	const open = persistence.open;
	if (typeof open === "function") {
		const handle = await open.call(persistence, sessionId, "read", { signal });
		try {
	...
	throw new Error("session persistence exposes no read API");
}
...
async function findSessionMeta(ctx, sessionId) {
	...
	for (const meta of await listStoredHeaders(ctx.get("sessionPersistence"))) {
...
async function lenientInspect(persistence, sessionId, signal) {
	try {
		return await readStoredSession(persistence, sessionId, signal);
	} catch (error) {
		if (typeof persistence.readRaw !== "function" || decodeStorageRecord === void 0) {
			// No raw-artifact path on this core (no readRaw, or the record codec has
			// moved away): a genuine miss is a 404, anything else stays the original
			// failure rather than being laundered into "session does not exist".
			if (isNotFoundLike(error)) throw sessionNotFoundError();
			throw error;
		}
...
async function buildDetails(ctx, sessionId) {
	...
	if (live !== void 0) {
		meta = live.header;
		// rc.8 exposed a plain `events` array on Session; 0.1.3-alpha.2 removed it in
		// favour of the public snapshotEvents() accessor. Spreading live.events there
		// throws "live.events is not iterable", which surfaces as a 500 in the detail
		// panel for the currently open (live) session.
		const liveEvents = typeof live.snapshotEvents === "function" ? live.snapshotEvents() : live.events;
		events = [...(liveEvents ?? [])];
	} else {
	...
	if (persistence !== void 0) {
		for (const h of await listStoredHeaders(persistence)) {
...
async function deleteSessionSingle(ctx, sessionId, options = {}) {
	...
	if (registry !== void 0 && typeof registry.requireState === "function" && typeof registry.setState === "function") {
		await enqueueMutation(async () => {
			...
			if (persistence !== void 0) {
				for (const h of await listStoredHeaders(persistence)) existing.add(h.id);
			}
	...
	if (sessionDirPath === void 0 && typeof persistence?.locate !== "function") {
		// No locate() on this backend: derive the record directory from the official
		// layout this module already mirrors, and only when it really exists.
		const derived = sessionDirFor(meta);
		const derivedRel = relative(sessionsRoot(), derived);
		if (derivedRel !== "" && derivedRel !== "." && !derivedRel.startsWith("..") && !isAbsolute(derivedRel)) {
			try {
				if ((await stat(derived)).isDirectory()) sessionDirPath = derived;
			} catch {
				// Not materialized yet: there is nothing to clean up.
			}
		}
	}
	...
	const removeLog = async () => {
		...
		if (sessionDirPath !== void 0) {
			// Neither locate() nor remove(): drop the record logs inside the derived
			// directory (the unified cleanup below removes the directory itself).
			// Naming is generation-flavoured on real cores: both session.jsonl.zstd
			// and session.v2.jsonl.zstd occur on disk, so match the session prefix plus
			// any known log/compression suffix (a bare .zst$ never matches ".zstd").
			for (const entry of await readdir(sessionDirPath).catch(() => [])) {
				if (/^session[^/]*\.(?:jsonl|zst|zstd|gz|br|lz4)$/i.test(entry)) {
					await Promise.resolve(rm(join(sessionDirPath, entry), { force: true, maxRetries: 3 })).catch(() => {});
				}
			}
		}
	};
...
async function cleanUpRetiredDirs(ctx, entries) {
	...
	for (const { sessionId, dir } of entries) {
		...
		// 0.1.3-alpha.2 has no coordinator.retirements to await, so a resurrection
		// by the retire tail-flush cannot be observed — retry instead. Bounded, and
		// the caller only ever resolves dir strictly inside sessionsRoot.
